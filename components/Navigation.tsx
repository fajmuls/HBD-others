...
import { Home, BookHeart, Image as ImageIcon, Gift, Info } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useState, useEffect } from 'react';
import VersionModal, { CURRENT_VERSION } from './VersionModal';
          ...
          >
            <div className="flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-2.5 md:py-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
              ...
              <button
                ...
                className="relative p-2 md:p-3 rounded-full flex items-center justify-center transition-colors text-white/30 hover:text-white/80 group"
                title="App Info"
              >
                <Info size={18} className="md:size-[22px] relative z-10 transition-transform group-hover:scale-110" />
                <span className="hidden md:block absolute -top-8 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  {CURRENT_VERSION}
                </span>
              </button>
            </div>
